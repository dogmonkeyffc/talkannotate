...
创建或追加文档版本时，推荐直接上传完整 Markdown 文件流：

- 新建文档：`POST /api/documents`
- 追加版本：`POST /api/documents/{id}`
- `title` 默认由服务端从 Markdown 内容推导：
  - 优先取第一个一级标题（`# `）
  - 否则取首个有效文本行并做定长截断
...
- 新文档首次发布：`POST /api/documents`，直接上传 Markdown 文件流（**前提是已通过 `GET /api/documents` 确认该文档不存在**）
- 已有文档再次发布：`POST /api/documents/{id}`，直接上传 Markdown 文件流
- 只拉取批注、不改正文：只读，不创建新版本
- 根据批注修改后再发布：生成完整新 Markdown，再追加版本（调用 `POST /api/documents/{id}`）
...
4. `POST /api/documents`（上传 Markdown 文件流）
...
2. 读取当前批注
3. 生成新的完整 Markdown
4. `POST /api/documents/{id}`，上传新的完整 Markdown 文件流
...
# 7) 根据批注发布新版本
curl -s -X POST http://localhost:3180/api/documents/<document-id> \
  -H 'Content-Type: text/markdown' \
  --data-binary @./release-notes.v2.md
```
