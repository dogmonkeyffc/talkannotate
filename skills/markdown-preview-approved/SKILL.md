...
为避免 agent 每次临时拼接脚本，仓库已提供可直接调用的命令：

- `pnpm preview:documents`
- `pnpm preview:publish -- --title "<title>" --file /absolute/path/to/doc.md [--id <document-id>]`
- `pnpm preview:document -- --id <document-id> [--version <n>]`
- `pnpm preview:versions -- --id <document-id>`
- `pnpm preview:annotations -- --id <document-id> [--version <n>]`
- `pnpm preview:set-changelog -- --id <document-id> --version <n> --file /absolute/path/to/change-log.md`

优先使用这些脚本，而不是在会话里现写 curl 或临时 Node/Python 脚本。
