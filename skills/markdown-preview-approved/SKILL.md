...
- 如果需要让评审者快速理解本次改动，发布成功后应补充该版本的 `changeLog`
...
- 上传成功拿到 `currentVersion` 后，如需补充更新日志，调用 `PUT /api/documents/{id}/versions/{version}/change-log`
...
5. 如有需要，基于返回的 `currentVersion` 调用 `PUT /api/documents/{id}/versions/{version}/change-log`
6. 如有需要，回查 `GET /api/documents/{id}/versions`
...
为避免 agent 每次临时拼接脚本，本 skill 自带一个可复制出去一起使用的 Python helper：`preview_api.py`。

- `python3 /path/to/markdown-preview-approved/preview_api.py documents`
- `python3 /path/to/markdown-preview-approved/preview_api.py publish --title "<title>" --file /absolute/path/to/doc.md [--id <document-id>]`
- `python3 /path/to/markdown-preview-approved/preview_api.py document --id <document-id> [--version <n>]`
- `python3 /path/to/markdown-preview-approved/preview_api.py versions --id <document-id>`
- `python3 /path/to/markdown-preview-approved/preview_api.py annotations --id <document-id> [--version <n>]`
- `python3 /path/to/markdown-preview-approved/preview_api.py set-changelog --id <document-id> --version <n> --file /absolute/path/to/change-log.md`

skill 使用方不需要知道本仓库目录，只要把整个 `markdown-preview-approved` 目录一起放到 skills 目录即可复用这个 helper。

优先使用这个 helper，而不是在会话里现写 curl 或临时脚本。
...
# 3.1) 为刚上传的版本补充更新日志
curl -s -X PUT http://localhost:3180/api/documents/<document-id>/versions/2/change-log \
  -H 'Content-Type: application/json' \
  -d '{
    "changeLog": "1. 新增接口说明\\n2. 修正评审流程"
  }'
